const { describe, test, expect } = require('@xufa/testing');
...
describe('Evaluator', () => {
  ...
  describe('Walk unary', () => {
    ...
    test('If the operator is unknown, return undefined', () => {
      const evaluator = new Evaluator();
  ...
  describe('Walk Binary', () => {
    ...
    test('If is an unknown operator throw an error', () => {
      ...
      const evaluator = new Evaluator();
      const question = 'true ^^ false';
      expect(() => evaluator.evaluate(question, context)).toThrow('Unexpected token (1:6)');
    });
  ...
  describe('Can use default context', () => {
    test('If no context is provided, use default context', () => {
      const context = { a: 12, b: 2 };
      const evaluator = new Evaluator(context);
      const question = 'if (a > 10) { c = 7; b++ } else { c = 3; b-- }';
      const result = evaluator.evaluate(question);
